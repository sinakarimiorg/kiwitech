                        ...
                        <div key={category._id}>
                            ...
                            <div className='flex items-center gap-2 sm:gap-3 px-4 sm:px-6 py-4'>
                                ...
                                <button
                                    ...
                                    className='flex items-center gap-1 sm:gap-2.5 flex-1 min-w-0 text-right cursor-pointer'>
                                    <span className='flex-center w-7 h-7 sm:w-9 sm:h-9 shrink-0 bg-primary-50 text-primary-600 rounded-lg'>
                                        <Icon className='w-3 h-3 sm:w-4.5 sm:h-4.5' />
                                    </span>
                                    <span className='font-IranYekanMedium text-xs md:text-sm text-zinc-700 line-clamp-1'>{category.title}</span>
                                    <span className='hidden sm:inline text-xs text-zinc-400 shrink-0'>({category.items.length} زیرمجموعه)</span>
                                    <PiCaretDownLight className={`w-3 md:w-4 h-3 md:h-4 text-zinc-400 shrink-0 transition-transform ${isOpen ? "rotate-180" : ""}`} />
                                </button>

                                <div className='flex items-center justify-end gap-1 sm:gap-3 shrink-0'>
                                    <button
                                        onClick={() => toggleActive(category._id, category.active)}
                                        className={`px-1.5 sm:px-2.5 py-0.75 text-[10px] sm:text-xs rounded-md md:rounded-lg transition-colors cursor-pointer whitespace-nowrap
                                            ${category.active ? "bg-primary-50 text-primary-600" : "bg-gray-100 text-zinc-400"}`}>
                                        {category.active ? "فعال" : "غیرفعال"}
                                    </button>
                                    <button
                                        onClick={() => openEditCategory(category)}
                                        className='flex-center w-6 md:w-8 h-6 md:h-8 text-zinc-500 hover:text-primary-600 hover:bg-primary-50 rounded-lg transition-colors cursor-pointer'>
                                        <PiPencilSimpleLight className='w-3 md:w-4 h-3 md:h-4' />
                                    </button>
                                    <button
                                        onClick={() => removeCategory(category._id)}
                                        className='flex-center w-6 md:w-8 h-6 md:h-8 text-zinc-500 hover:text-danger hover:bg-danger/10 rounded-lg transition-colors cursor-pointer'>
                                        <PiTrashLight className='w-3 md:w-4 h-3 md:h-4' />
                                    </button>
                                </div>
                            </div>
                                ...
                                <div className='bg-gray-50/60 px-4 sm:px-6 py-4 pr-8 sm:pr-20'>
                                    <div className='flex flex-col gap-2'>
                                        {category.items.map(item => (
                                            <div key={item._id} className='flex items-center gap-3 bg-white px-4 py-1 md:py-2.5 border border-gray-100 rounded-lg'>
                                                <PiDotsSixVerticalLight className='hidden sm:block w-4 h-4 text-zinc-300 cursor-grab shrink-0' />
                                                <span className='flex-1 text-[10px] md:text-sm text-zinc-600 min-w-0 truncate'>{item.title}</span>
                                                <button
                                                    onClick={() => setItemModalState({ categoryId: category._id, itemId: item._id, title: item.title })}
                                                    className='flex-center w-5 md:w-7 h-5 md:h-7 text-zinc-400 hover:text-primary-600 hover:bg-primary-50 rounded-md transition-colors cursor-pointer shrink-0'>
                                                    <PiPencilSimpleLight className='w-3.5 h-3.5' />
                                                </button>
                                                <button
                                                    onClick={() => removeItem(category._id, item._id)}
                                                    className='flex-center w-5 md:w-7 h-5 md:h-7 text-zinc-400 hover:text-danger hover:bg-danger/10 rounded-md transition-colors cursor-pointer shrink-0'>
                                        ...
                                        {category.items.length === 0 &&
                                            <p className='py-1 md:py-2 text-[10px] md:text-xs text-zinc-400 text-center'>زیرمجموعه‌ای ثبت نشده است.</p>
                                        }
                                    </div>

                                    <button
                                        onClick={() => setItemModalState({ categoryId: category._id })}
                                        className='flex-center gap-1.5 w-full mt-3 py-1 md:py-2.5 text-xs md:text-sm text-primary-600 hover:text-primary-700 border border-dashed border-primary-300 hover:border-primary-400 rounded-lg transition-colors cursor-pointer'>
                                        <PiPlusCircleLight className='w-4 h-4' />
                                        افزودن زیرمجموعه
                                    </button>
                                </div>
